...
<div class="pd-20">
	...
	<div  class="cl pd-12 bg-1 bk-gray">
		<c:forEach items="${sysFrontlinkVos}" var="sysFrontlinkVo">
			<div class="check-box">
				<input type="checkbox" id="roles" name="roles" value="${sysFrontlinkVo.id}" <c:if test="${sysFrontlinkVo.flag ==true}">checked</c:if>>
				<label for="roles">${sysFrontlinkVo.linkName}:${sysFrontlinkVo.linkUrl}</label>
			</div>
		</c:forEach>
	</div>
...
<script type="text/javascript">
    $("#submit").bind("click",function () {
        ...
        for(var i=0;i<roleObj.length;i++){
            if(i==0){
                linkIds = $(roleObj[i]).val();
			}else{
                linkIds += ","+$(roleObj[i]).val();
			}
		}
        $.ajax( {
            url:'${basePath}/${url}/distributionSave',// 跳转到 action
            type:'post',
            cache:false,
            data:{"roleId":${roleId},"linkIds":linkIds},
            dataType:'json',
