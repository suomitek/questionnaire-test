	...
	<form class="form form-horizontal" id="submintInfo" method="post" >
		<div class="row cl">
			...
			<div class="formControls col-sm-8 col-xs-8">
				<input id="resName" name="resName" value="${sysResource.resName}" class="input-text" placeholder="请输入菜单名称" type="text">
				<input id="parentId" name="parentId" type="hidden" value="${sysResource.parentId}">
				<input id="resType" name="resType" type="hidden" value="${sysResource.resType}">
		...
		<div class="row cl">
			...
			<div class="formControls col-sm-8 col-xs-8">
				<input id="resCode" name="resCode" value="${sysResource.resCode}" class="input-text" placeholder="请输入菜单编码" type="text">
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-sm-2 col-xs-3" style="text-align: right"><span class="c-red">*</span>菜单路径：</label>
			<div class="formControls col-sm-8 col-xs-8">
				<input id="resUrl" name="resUrl" value="${sysResource.resUrl}" class="input-text" placeholder="请输入菜单路径" type="text">
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-sm-2 col-xs-3" for="orderNum" style="text-align: right"><span class="c-red">*</span>菜单排序号：</label>
			<div class="formControls col-sm-8 col-xs-8">
				<input id="orderNum" name="orderNum" value="${sysResource.orderNum}" class="input-text" placeholder="请输入菜单排序号" type="text">
			</div>
		</div>
		<div class="row cl <c:if test="${sysResource.resType == 1}">hide</c:if>">
			...
			<div class="formControls col-sm-8 col-xs-8">
				<select name="btnFlag" id="btnFlag" class="select input-text">
					<option value="1" <c:if test="${sysResource.btnFlag == 1}">selected = "selected"</c:if>>页面资源</option>
					<option value="0" <c:if test="${sysResource.btnFlag == 0}">selected = "selected"</c:if>>按钮资源</option>
				</select>
			</div>
		</div>

		<div class="row cl" id="apis">
...
<script type="text/javascript">
    $().ready(function() {
        ...
        <c:if test="${sysResource.btnFlag == 0}">
        		$("#apis").hide();
		</c:if>
        ...
        jQuery("#submintInfo").validate({
            ...
            submitHandler: function(form) {
                ...
                $.ajax( {
                    url:'${basePath}/manage/admin/resource/editSave',// 跳转到 action
                    type:'post',
                    cache:false,
