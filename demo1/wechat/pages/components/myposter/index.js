  ...
  methods: {
    ...
    sharePosteCanvas: function(avaterSrc, codeSrc, imgInfo) {
      ...
      query.select('#canvas-container').boundingClientRect(function(rect) {
        ...
        if (that.data.time || that.data.time == 0) {
          ctx.setFontSize(11);
          ctx.setFillStyle('#737373');
          ctx.setTextAlign('left');
          var time = that.data.time;
          ctx.fillText(time, left - 15, imgheght + 110); //电话
        }


        //  绘制二维码
        if (codeSrc) {
          ctx.drawImage(codeSrc, left + 215, imgheght + 10, width / 4, width / 4)
          ctx.setFontSize(10);
          ctx.setFillStyle('#000');
          ctx.fillText("长按保存图片", left + 230, imgheght + 110);
        }
      }).exec()
      setTimeout(function() {
        ctx.draw();
